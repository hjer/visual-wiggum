...
- [x] In `partials/dashboard_content.html`, add a collapsible "Specs" section between active items and "Implementation Plan" (same pattern as plan section)
- [x] Show aggregate progress for specs section `(done/total)`
- [x] Update archive section to sub-group archived specs under dimmed "Specs" heading (matching plan sub-group pattern)
- [x] Add tests for specs section presence and content in web dashboard
- [x] Definition of done: Web dashboard shows Active → Specs (collapsible) → Plan (collapsible) → Archive (collapsed, sub-grouped)
